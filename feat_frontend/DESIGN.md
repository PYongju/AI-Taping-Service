...
Toss is Korea's fintech super-app that redefined what a financial interface could feel like -- calm, confident, and deceptively simple. The page opens on a clean white canvas (`#ffffff`) with deep charcoal headings (`#191f28`) and a signature blue (`#f97316`) that functions as the universal interactive accent. This isn't the cold, institutional blue of legacy banking; it's a bright, optimistic cerulean that says "your money is in good hands, and we'll make it easy."
...
**Key Characteristics:**
- Toss Blue (`#f97316`) as the primary interactive color -- bright, optimistic, trustworthy
...
### Primary
- **Toss Blue** (`#f97316`): `blue500`. Primary interactive color -- CTAs, links, active states, selection highlights. The workhorse of every tappable element.
...
- Text: `#f97316` (blue500) or `#191f28` (grey900)
...
- Radius: 12px (customized), 16px (featured), 8px (compact)
...
- Border: 1px solid `#e5e8eb`, focus: 2px solid `#f97316`
- Radius: 12px
...
- Active: `#f97316` icon + `#191f28` text, Inactive: `#b0b8c1` icon + `#8b95a1` text
...
### Do
- Use Toss Blue (`#f97316`) for all interactive elements -- links, buttons, toggles, selections
...
### Don't
- Don't confuse Brand Blue (`#0064FF`) with UI Blue (`#f97316`) -- brand is for marketing/logo only
...
- "Build a send-money button: #f97316 bg, white text, 16px weight 600, min-height 56px, 12px radius, full-width. Pressed: overlay dim. Loading: 3-dot white animation."
...
- "Create an OTP input: 6 boxes, each 48px wide, 56px tall, 8px radius, 1px border #e5e8eb. Active: 2px border #f97316. Digit: 24px weight 700, centered, #191f28."
- "Design a bottom tab bar: white bg, top border 1px #e5e8eb. 4 tabs evenly spaced. Active: #f97316 icon + #191f28 label 11px weight 500. Inactive: #b0b8c1 icon + #8b95a1 label. Tab height 56px with safe area."

### Iteration Guide
1. Always use the full Toss Product Sans font stack with Korean fallbacks
2. Primary interactive color is `#f97316` (blue500) -- never `#0064FF` (brand blue)
...
@layer base {
  :root {
    --background: 0 0% 100%;
    --foreground: 216 23% 13%;
    --card: 0 0% 100%;
    --card-foreground: 216 23% 13%;
    --popover: 0 0% 100%;
    --popover-foreground: 216 23% 13%;
    --primary: 25 95% 53%;
    --primary-foreground: 0 0% 98%;
    --secondary: 25 94% 94%;
    --secondary-foreground: 216 23% 13%;
    --muted: 0 0% 96%;
    --muted-foreground: 216 23% 53%;
    --accent: 55 60% 55%;
